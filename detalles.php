...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    ...
    <section class="principal">
        ...
        <center><h1>Datos de la Orden</h1></center>
            <?PHP
                include('establecerconexion.php');
                $numeroventa=0;
                $consulta=mysql_query("SELECT * FROM `t_car` WHERE `ord_id`=$_GET[orden]");
                while($row = mysql_fetch_array($consulta)){
            ?>
            <?PHP if($numeroventa !=$row['ord_id']){
            ?> 
                </article>
                </article>
                <article class="nordeen">
                    <b>Orden</b> <?PHP echo $row['ord_id'];
                    $qqw=mysql_query("SELECT * FROM `t_ord` WHERE `t_ord`.`ord_id`=$_GET[orden]");
                    $tt=mysql_fetch_array($qqw);
                    $total=$tt['ord_tot'];
                    $total_arreglado= number_format( $total , 2 , "," ,"." );
                    echo "<article class='totalizer'>Total de la orden Bs.".$total_arreglado."</article>";
                    ?><br><br>
                <article class="prod">
                    <?PHP
            }
                    $con=mysql_query("SELECT * FROM `t_pro` WHERE `pro_id`=$row[pro_id]");
                    $g=mysql_fetch_array($con);
                    $u=mysql_query("SELECT * FROM `t_car` WHERE `pro_id`=$row[pro_id] AND `ord_id`=$_GET[orden]");
                    $uj=mysql_fetch_array($u);
                    $numeroventa=$row['ord_id'];
                    $precio=$g['pro_pre'];
                    $precio_arreglado= number_format( $precio , 2 , "," ,"." );
                    echo "<img src='productos/".$g['pro_img']."' width='80px'>
                            <a href='producto.php?pro_id=".$g['pro_id']."'><article class='produ'>".$g['pro_pro']." <div style='float:right;'>Bs.".$precio_arreglado."&nbsp;&nbsp;x&nbsp;&nbsp;".$uj['car_can']."</div></article></a>
                    ";
            }
            ?>
        </section>
        ...
        <article class="nordeen">
            <?PHP
                    $qqw=mysql_query("SELECT * FROM `t_ord` WHERE `t_ord`.`ord_id`=$_GET[orden]");
                    $tt=mysql_fetch_array($qqw);
                    echo "<span>Cliente:&nbsp;&nbsp; $tt[ord_emp]</span><br>";
                    echo "<span>RIF:&nbsp;&nbsp; $tt[ord_rif]</span><br>";
                    echo "<span>Direcci&oacute;n:&nbsp;&nbsp; $tt[ord_dir]</span><br>";
                    echo "<span>Telefono:&nbsp;&nbsp; $tt[ord_tel]</span><br>";
                    echo "<span>Correo:&nbsp;&nbsp; $tt[ord_cor]</span><br>";
            ?>
        </article>
